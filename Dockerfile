...
ADD . $SRC_DIR
RUN cd $SRC_DIR; pwd;cd handler;go test -v ;go build -o benandjerry  -ldflags "-X 'main.buildTimestamp=$(date '+%b %d %Y %T')' -X main.commitID=`git describe --match=rAnDom --always --abbrev --dirty`"; cp benandjerry /ben-and-jerry/
